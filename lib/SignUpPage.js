...
var captchaNode = config.captchaNode,
    sessionNode = config.sessionNode
    ...
    return (req, res, parsedUrl) => {
        ...
        ;(() => {
            ...
            function errorListener (err) {
                console.log('ERROR: captcha-node-client: verify: ' + JSON.stringify(err))
                Error500Page(res)
            }
            ...
            var proxyReq = http.request({
                ...
                if (statusCode !== 200) {
                    console.log('ERROR: captcha-node-client: verify: HTTP status code ' + statusCode)
                    Error500Page(res)
                    return
                }
                ...
                proxyRes.on('end', () => {
                    ...
                    if (response !== true) {
                        console.log('ERROR: captcha-node-client: verify: Invalid response ' + JSON.stringify(response))
                        Error500Page(res)
                        return
                    }
