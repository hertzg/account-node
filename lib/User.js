    ...
    return {
        ...
        addReferer: (refererUsername, profile) => {
            ...
            if (contact === undefined) {
                ...
                castSessionMessage(['editContactProfile', [refererUsername, profile]])
                if (Object.keys(sessions).length !== 0) {
                    SendContactOnline(username, refererUsername)
                }
                SendEditContactProfile(username, refererUsername, profile)
            }
        ...
        editContactProfile: (contactUsername, profile) => {
            assert.notStrictEqual(contacts[contactUsername], undefined)
            contacts[contactUsername].profile = profile
            castSessionMessage(['editContactProfile', [contactUsername, profile]])
        },
        editProfile: (_profile, token) => {
            ...
            if (affectedContacts.length > 0) {
                CastContactProfile(username, profile)
            }
