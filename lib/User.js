...
module.exports = (password, profile, registerTime,
    ...
    return {
        addContact: (username, contactUsername, profile, token) => {
            assert.strictEqual(contacts[contactUsername], undefined)
            contacts[contactUsername] = profile
            sendMessage(username, ['addContact', [contactUsername, profile]], token)
            AddReferer(contactUsername, username, publicProfile)
            if (requests[contactUsername] !== undefined) {
                delete requests[contactUsername]
                sendMessage(username, ['removeRequest', contactUsername], token)
            }
        },
        ...
        ignoreRequest: (username, requestUsername, token) => {
            ...
            sendMessage(username, ['removeRequest', requestUsername], token)
