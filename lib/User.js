...
module.exports = (password, profile, registerTime,
    ...
    function castSessionMessage (username, message, token) {
    ...
    return {
        addContact: (username, contactUsername, contact, token) => {
            ...
            castSessionMessage(username, ['addContact', [contactUsername, contact]], token)
        ...
        addReferer: (username, refererUsername, profile, token) => {
            ...
            if (contacts[refererUsername] === undefined &&
                ...
                castSessionMessage(username, ['addRequest', [refererUsername, profile]])

            }

        },
        addSession: (username, token, session) => {
        ...
        contactOffline: (username, contactUsername) => {
            assert.notStrictEqual(contacts[contactUsername], undefined)
            contacts[contactUsername].online = false
            castSessionMessage(username, ['offline', contactUsername])
        },
        contactOnline: (username, contactUsername) => {
            assert.notStrictEqual(contacts[contactUsername], undefined)
            contacts[contactUsername].online = true
            castSessionMessage(username, ['online', contactUsername])
        },
        editContact: (username, contactUsername, profile, token) => {
            assert.notStrictEqual(contacts[contactUsername], undefined)
            contacts[contactUsername].profile = profile
            castSessionMessage(username, ['editContact', [contactUsername, profile]], token)
        },
        editProfile: (username, _profile, token) => {
            ...
            castSessionMessage(username, ['editProfile', profile], token)
        ...
        ignoreRequest: (username, requestUsername, token) => {
            ...
            castSessionMessage(username, ['ignoreRequest', requestUsername], token)
        ...
        removeContact: (username, contactUsername, token) => {
            ...
            castSessionMessage(username, ['removeContact', contactUsername], token)
        ...
        removeReferer: (username, refererUsername, token) => {
            ...
            castSessionMessage(username, ['removeReferer', refererUsername], token)
        },
        removeSession: (username, token) => {
