...
module.exports = (username, password, profile, registerTime,
    sessions, contacts, referers, requests, ignoreds, longTermSessions) => {
    ...
    assert(IsPlainObject(longTermSessions))
    ...
    return {
        ...
        addLongTermSession: (token, session) => {
            assert.strictEqual(longTermSessions[token], undefined)
            longTermSessions[token] = session
        },
        ...
        getLongTermSession: token => {
            return longTermSessions[token]
        },
        getProfile: () => {
            return profile
        },
        ...
        removeLongTermSession: token => {
            assert.notStrictEqual(longTermSessions[token], undefined)
            delete longTermSessions[token]
        },
        ...
        toStorageObject: () => {
            ...
            var storageLongTermSessions = {}
            for (var i in longTermSessions) {
                storageLongTermSessions[i] = longTermSessions[i].toStorageObject()
            }
            ...
            return {
                ...
                requests: requests,
                ignoreds: ignoreds,
                longTermSessions: storageLongTermSessions,
            }
